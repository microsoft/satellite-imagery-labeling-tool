...
1. Open the [labeling tool](https://microsoft.github.io/satellite-imagery-labeling-tool/src/labeler.html). 
   - **Note:** Your project adminstrator may have provided an link that will open up your specific task or may point to a forked version of this tool.
...
All cached data will be **deleted after 30 days** by default (you can adjust this in the `labeler_settings.js` file under `autoSave -> ttl`). All data stored within the end users browser and is not sent to any servers for security and privacy.
